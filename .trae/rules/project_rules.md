...
- **格式**：`- 分类：面向使用用户（小程序使用者不是开发者）的变更描述`
...
所有方案输出（代码修改、架构调整、内容优化、流程改进等）必须包含以下四个要素：

### 12.1 方案是什么
一句话说清楚方案的核心内容。

### 12.2 怎么修改
具体的改动路径——改了哪些文件、改了什么、怎么改的。要有可操作性，不能只讲抽象概念。

### 12.3 造成什么影响和后果
改动的前后对比。会影响到哪些功能？有没有副作用？有没有需要额外注意的地方（比如数据迁移、兼容性、回退方案）？

### 12.4 改进了什么 / 优化了什么
这个方案的价值落脚点——性能提升、可维护性、用户体验、代码整洁度、架构清晰度等。量化最好，定性也行。

## 13. 暗黑模式适配规范

所有 UI 界面的生成（包括但不限于新页面、新功能、卡片、弹窗、组件、主题颜色等）必须适配暗黑模式。

### 13.1 核心原则
- **不允许硬编码色值**。所有颜色必须通过 CSS 变量（Custom Properties）或小程序 `@media (prefers-color-scheme: dark)` 机制控制
- **不允许只做亮色不做暗色**。任何新增 UI 元素在提交前必须验证暗色模式下的可读性和视觉效果

### 13.2 实现方式
- 使用微信小程序原生暗黑模式适配方案：通过 `app.json` 配置 `"darkmode": true`，利用 CSS 变量或 WXSS 的 `@media (prefers-color-scheme: dark)` 切换
- 或在组件级别定义暗色变量映射，确保一处定义全局生效

### 13.3 检查清单
- 文字在暗色背景下有足够对比度（至少 4.5:1）
- 图片/图标在暗色模式下有替代版本或使用可自适应颜色格式
- 卡片、边框、阴影在暗色模式下不丢失层次感
- 输入框、按钮、选中态等交互元素在暗色模式下状态清晰可辨
- 新增颜色变量须同时在亮色/暗色两组变量中定义
