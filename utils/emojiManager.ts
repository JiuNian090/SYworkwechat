'use strict';

interface EmojiCategory {
  id: string;
  name: string;
  icon: string;
}

type EmojiEmotion = 'positive' | 'neutral' | 'negative';

const emojiCategories: EmojiCategory[] = [
  { id: 'face', name: '表情', icon: '😊' },
  { id: 'animal', name: '动物', icon: '🐶' },
  { id: 'food', name: '食物', icon: '🍎' },
  { id: 'activity', name: '活动', icon: '🏃' },
  { id: 'travel', name: '旅行', icon: '🚗' },
  { id: 'object', name: '物品', icon: '⚽' }
];

const emojiData: Record<string, string[]> = {
  face: ['😊', '😃', '😄', '😁', '😆', '😂', '🤣', '😅', '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😚', '😋', '😛', '😝', '😜', '🤪', '😎', '🤩', '🥳', '😏', '🤓', '🧐', '🤨', '🤔', '🤗', '🤭', '😮', '😯', '😲', '😧', '😦', '😨', '😱', '😖', '😣', '😫', '😩', '🥺', '😢', '😭', '😤', '😠', '😡', '🤬', '😳', '🥵', '🥶', '😴', '😪', '🤤', '😓', '😟', '😔', '😞', '😒', '🙁', '☹️', '😕', '🤫', '😶', '😐', '😑', '😬', '🙄', '😵', '🤐', '🥴', '🤯', '🤥', '🤢', '🤮', '🤧', '😷', '🤒', '🤕', '🤑'],
  animal: ['🐶', '🐱', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮', '🐷', '🐸', '🐵', '🐔', '🐧', '🐦', '🐤', '🦋', '🐝', '🐛', '🦄', '🐙', '🐠', '🐟', '🐬', '🐳', '🐋', '🦈', '🐊', '🐢', '🐍', '🦎', '🦖', '🦕', '🐘', '🦛', '🦏', '🐪', '🐫', '🦒', '🦘', '🐃', '🐂', '🐄', '🐎', '🐖', '🐏', '🐑', '🐐', '🐓', '🦃', '🦅', '🦆', '🦉', '🦤', '🪶', '🦩', '🦚', '🦜', '🐿️', '🦔', '🐇', '🐀', '🐁'],
  food: ['🍎', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🫐', '🍈', '🍒', '🍑', '🥭', '🍍', '🥥', '🥝', '🍅', '🫒', '🥑', '🍆', '🥔', '🥕', '🌽', '🌶️', '🫑', '🥒', '🥬', '🥦', '🧄', '🧅', '🍄', '🥜', '🫘', '🌰', '🫚', '🫛', '🍞', '🥐', '🥖', '🫓', '🥨', '🥯', '🥞', '🧇', '🧀', '🍖', '🍗', '🥩', '🥓', '🍔', '🍟', '🍕', '🌭', '🥪', '🌮', '🌯', '🫔', '🥙', '🧆', '🥚', '🍳', '🥘', '🍲', '🫕', '🥣', '🥗', '🍿', '🧈', '🧂', '🥫', '🍱', '🍘', '🍙', '🍚', '🍛', '🍜', '🍝', '🍠', '🍢', '🍣', '🍤', '🍥', '🦪', '🍦', '🍧', '🍨', '🍩', '🍪', '🎂', '🍰', '🧁', '🥧', '🍫', '🍬', '🍭', '🍮', '🍯', '🍼', '🥛', '🫖', '🍵', '🍶', '🍾', '🍷', '🍸', '🍹', '🍺', '🍻', '🥂', '🥃', '🫗', '🥤', '🧋', '🧃', '🧉', '🧊', '🥢', '🍽️', '🍴', '🥄', '🔪'],
  activity: ['🏃', '🚶', '🧎', '🧍', '🚶‍♀️', '🚶‍♂️', '🏃‍♀️', '🏃‍♂️', '🧎‍♀️', '🧎‍♂️', '🧍‍♀️', '🧍‍♂️', '🧑‍🦯', '👨‍🦯', '👩‍🦯', '🧑‍🦼', '👨‍🦼', '👩‍🦼', '🧑‍🦽', '👨‍🦽', '👩‍🦽', '🏌️', '🏌️‍♀️', '🏌️‍♂️', '🏋️', '🏋️‍♀️', '🏋️‍♂️', '🏊', '🏊‍♀️', '🏊‍♂️', '🏄', '🏄‍♀️', '🏄‍♂️', '🚣', '🚣‍♀️', '🚣‍♂️', '🧗', '🧗‍♀️', '🧗‍♂️', '🚵', '🚵‍♀️', '🚵‍♂️', '🚴', '🚴‍♀️', '🚴‍♂️', '🏇', '🏇‍♀️', '🏇‍♂️', '⛷️', '🎿', '🏂', '🤸', '🤸‍♀️', '🤸‍♂️', '🤽', '🤽‍♀️', '🤽‍♂️', '🤾', '🤾‍♀️', '🤾‍♂️', '🤹', '🤹‍♀️', '🤹‍♂️', '🎪', '🎭', '🎨', '🎬', '🎤', '🎧', '🎼', '🎹', '🥁', '🎷', '🎺', '🎸', '🪕', '🎻', '🪗', '🎯', '🎱', '🎳', '🎮', '🎰', '🎲', '🧩'],
  travel: ['🚗', '🚕', '🚙', '🚌', '🚎', '🏎️', '🚓', '🚑', '🚒', '🚐', '🚚', '🚛', '🚜', '🛵', '🏍️', '🛺', '🚲', '🛴', '🚨', '🚔', '🚍', '🚘', '🚖', '🚡', '🚠', '🚟', '🚃', '🚋', '🚞', '🚝', '🚄', '🚅', '🚈', '🚂', '🚆', '🚇', '🚊', '⛽', '🚧', '🚦', '🚥', '🛞', '🚀', '🛸', '🚁', '🛩️', '✈️', '🛫', '🛬', '🪂', '🚤', '🛳️', '⛴️', '🚢', '⚓', '⛰️', '🌋', '🏔️', '🗻', '🏕️', '🏖️', '🏜️', '🏝️', '🏞️', '🏟️', '🏛️', '🏗️', '🧱', '🪨', '🪵', '🛖', '🏘️', '🏚️', '🏠', '🏡', '🏢', '🏬', '🏭', '🏯', '🏰', '💒', '🗼', '🗽', '⛪', '🕌', '🛕', '🕍', '⛩️', '🛤️', '🛢️', '💈', '⛲', '🎡', '🎢', '🎠', '⛱️', '🎃', '🎄', '🎆', '🎇', '🧨', '✨', '🎈', '🎉', '🎊', '🎋', '🎍', '🎎', '🎏', '🎐', '🎑', '🧧', '🎀', '🎁', '🤶', '🎅', '🦌', '⛄', '🔥', '🎄', '❄️', '☃️', '🌨️', '🌧️', '🌦️', '🌥️', '☁️', '🌤️', '☀️', '🌝', '🌞', '🪐', '🌟', '⭐', '✨', '⚡', '☄️', '💫', '🌙', '🌚', '🌛', '🌜', '🌔', '🌓', '🌒', '🌑', '🌘', '🌗', '🌖', '🌕', '🌪️', '🌫️', '🌬️', '🌀', '🌈', '🌂', '☂️', '🌊', '💧', '💦', '☔'],
  object: ['🏵️', '🎗️', '🎟️', '🎫', '🎖️', '🏆', '🏅', '🥇', '🥈', '🥉', '⚽', '⚾', '🥎', '🏀', '🏐', '🏈', '🏉', '🎾', '🥏', '🎳', '🏏', '🏑', '🏒', '🥍', '🏓', '🏸', '🥊', '🥋', '🥅', '🎣', '🎽', '🎿', '🛷', '🎯', '🎱', '🔮', '🎰', '🎲', '🧩', '🎸', '🎹', '🎺', '🎻', '🥁', '🎤', '🎧', '🎼', '🎵', '🎶', '🎹', '🎷', '🎸', '🪕', '🎻', '🪗', '🎯', '🎱', '🎳', '🎮', '🎰', '🎲', '🧩']
};

const emojiTextMap: Record<string, string> = {
  '😊': '微笑',
  '😃': '开心',
  '😄': '大笑',
  '😁': '露齿笑',
  '😆': '大笑',
  '😅': '汗颜',
  '🤣': '捧腹大笑',
  '😂': '笑哭',
  '🙂': '略微微笑',
  '🙃': '倒脸',
  '😉': '眨眼',
  '😌': '安心',
  '😍': '爱心眼',
  '🥰': '爱慕',
  '😘': '飞吻',
  '😚': '闭唇亲吻',
  '😋': '美味',
  '😛': '吐舌',
  '😝': '调皮吐舌',
  '😜': '眨眼吐舌',
  '🤪': '搞怪',
  '😎': '酷',
  '🤩': '崇拜',
  '🥳': '庆祝',
  '😏': '得意',
  '🤓': '书呆子',
  '🧐': '思考',
  '🤨': '挑眉',
  '🤔': '思考',
  '🤗': '拥抱',
  '🤭': '捂嘴笑',
  '😮': '惊讶',
  '😯': '惊讶',
  '😲': '震惊',
  '😧': '惊恐',
  '😦': '震惊',
  '😨': '害怕',
  '😱': '尖叫',
  '😖': '折磨',
  '😣': '痛苦',
  '😫': '压力',
  '😩': '好累',
  '🥺': '恳求',
  '😢': '哭泣',
  '😭': '痛哭',
  '😤': '生气',
  '😠': '愤怒',
  '😡': '暴怒',
  '🤬': '暴怒',
  '😳': '脸红',
  '🥵': '热',
  '🥶': '冷',
  '😴': '睡觉',
  '😪': '困倦',
  '🤤': '流口水',
  '😓': '汗',
  '😟': '担心',
  '😔': '难过',
  '😞': '失望',
  '😒': '无语',
  '🙁': '沮丧',
  '☹️': '不满',
  '😕': '困惑',
  '🤫': '安静',
  '😶': '无语',
  '😐': '中性',
  '😑': '无奈',
  '😬': '尴尬',
  '🙄': '翻白眼',
  '😵': '晕眩',
  '🤐': '闭嘴',
  '🥴': '迷糊',
  '🤯': '爆炸',
  '🤥': '说谎',
  '🤢': '恶心',
  '🤮': '呕吐',
  '🤧': '打喷嚏',
  '😷': '戴口罩',
  '🤒': '发烧',
  '🤕': '受伤',
  '🤑': '金钱眼',

  '🐶': '狗',
  '🐱': '猫',
  '🐭': '老鼠',
  '🐹': '仓鼠',
  '🐰': '兔子',
  '🦊': '狐狸',
  '🐻': '熊',
  '🐼': '熊猫',
  '🐨': '考拉',
  '🐯': '老虎',
  '🦁': '狮子',
  '🐮': '牛',
  '🐷': '猪',
  '🐸': '青蛙',
  '🐵': '猴子',
  '🐔': '鸡',
  '🐧': '企鹅',
  '🐦': '鸟',
  '🐤': '小鸡',
  '🦋': '蝴蝶',
  '🐝': '蜜蜂',
  '🐛': '毛毛虫',
  '🦄': '独角兽',
  '🐙': '章鱼',
  '🐠': '鱼',
  '🐟': '鱼',
  '🐬': '海豚',
  '🐳': '鲸鱼',
  '🐋': '鲸鱼',
  '🦈': '鲨鱼',
  '🐊': '鳄鱼',
  '🐢': '乌龟',
  '🐍': '蛇',
  '🦎': '蜥蜴',
  '🦖': '恐龙',
  '🦕': '恐龙',
  '🐘': '大象',
  '🦛': '河马',
  '🦏': '犀牛',
  '🐪': '骆驼',
  '🐫': '骆驼',
  '🦒': '长颈鹿',
  '🦘': '袋鼠',
  '🐃': '牛',
  '🐂': '牛',
  '🐄': '牛',
  '🐎': '马',
  '🐖': '猪',
  '🐏': '羊',
  '🐑': '羊',
  '🐐': '羊',
  '🐓': '鸡',
  '🦃': '火鸡',
  '🦅': '鹰',
  '🦆': '鸭子',
  '🦉': '猫头鹰',
  '🦤': '鸟',
  '🪶': '羽毛',
  '🦩': '火烈鸟',
  '🦚': '孔雀',
  '🦜': '鹦鹉',
  '🐿️': '松鼠',
  '🦔': '刺猬',
  '🐇': '兔子',
  '🐀': '老鼠',
  '🐁': '老鼠',

  '🍎': '苹果',
  '🍊': '橘子',
  '🍋': '柠檬',
  '🍌': '香蕉',
  '🍉': '西瓜',
  '🍇': '葡萄',
  '🍓': '草莓',
  '🫐': '蓝莓',
  '🍈': '瓜',
  '🍒': '樱桃',
  '🍑': '桃子',
  '🥭': '芒果',
  '🍍': '菠萝',
  '🥥': '椰子',
  '🥝': '猕猴桃',
  '🍅': '番茄',
  '🫒': '橄榄',
  '🥑': '牛油果',
  '🍆': '茄子',
  '🥔': '土豆',
  '🥕': '胡萝卜',
  '🌽': '玉米',
  '🌶️': '辣椒',
  '🫑': '甜椒',
  '🥒': '黄瓜',
  '🥬': '青菜',
  '🥦': '西兰花',
  '🧄': '大蒜',
  '🧅': '洋葱',
  '🍄': '蘑菇',
  '🥜': '花生',
  '🫘': '豆子',
  '🌰': '栗子',
  '🫚': '姜',
  '🫛': '豌豆',
  '🍞': '面包',
  '🥐': '羊角面包',
  '🥖': '法棍',
  '🫓': '饼',
  '🥨': '椒盐卷饼',
  '🥯': '贝果',
  '🥞': '煎饼',
  '🧇': '华夫饼',
  '🧀': '奶酪',
  '🍖': '肉',
  '🍗': '鸡腿',
  '🥩': '牛排',
  '🥓': '培根',
  '🍔': '汉堡',
  '🍟': '薯条',
  '🍕': '披萨',
  '🌭': '热狗',
  '🥪': '三明治',
  '🌮': '塔可',
  '🌯': '卷饼',
  '🫔': '墨西哥卷',
  '🥙': '皮塔饼',
  '🧆': '法拉费',
  '🥚': '鸡蛋',
  '🍳': '煎蛋',
  '🥘': '炖菜',
  '🍲': '汤',
  '🫕': '汤',
  '🥣': '粥',
  '🥗': '沙拉',
  '🍿': '爆米花',
  '🧈': '黄油',
  '🧂': '盐',
  '🥫': '罐头',
  '🍱': '便当',
  '🍘': '米饼',
  '🍙': '饭团',
  '🍚': '米饭',
  '🍛': '咖喱饭',
  '🍜': '面条',
  '🍝': '意大利面',
  '🍠': '红薯',
  '🍢': '串烧',
  '🍣': '寿司',
  '🍤': '天妇罗',
  '🍥': '鱼板',
  '🦪': '生蚝',
  '🍦': '冰淇淋',
  '🍧': '刨冰',
  '🍨': '甜点',
  '🍩': '甜甜圈',
  '🍪': '饼干',
  '🎂': '蛋糕',
  '🍰': '蛋糕',
  '🧁': '纸杯蛋糕',
  '🥧': '派',
  '🍫': '巧克力',
  '🍬': '糖果',
  '🍭': '棒棒糖',
  '🍮': '布丁',
  '🍯': '蜂蜜',
  '🍼': '奶瓶',
  '🥛': '牛奶',
  '🫖': '茶壶',
  '🍵': '茶',
  '🍶': '酒',
  '🍾': '香槟',
  '🍷': '红酒',
  '🍸': '鸡尾酒',
  '🍹': '果汁',
  '🍺': '啤酒',
  '🍻': '啤酒',
  '🥂': '香槟',
  '🥃': '威士忌',
  '🫗': '倒酒',
  '🥤': '饮料',
  '🧋': '奶茶',
  '🧃': '果汁',
  '🧉': '马黛茶',
  '🧊': '冰块',
  '🥢': '筷子',
  '🍽️': '餐具',
  '🍴': '叉子',
  '🥄': '勺子',
  '🔪': '刀',

  '🏃': '跑步',
  '🚶': '步行',
  '🧎': '跪',
  '🧍': '站',
  '🚶‍♀️': '女步行',
  '🚶‍♂️': '男步行',
  '🏃‍♀️': '女跑步',
  '🏃‍♂️': '男跑步',
  '🧎‍♀️': '女跪',
  '🧎‍♂️': '男跪',
  '🧍‍♀️': '女站',
  '🧍‍♂️': '男站',
  '🧑‍🦯': '导盲',
  '👨‍🦯': '男导盲',
  '👩‍🦯': '女导盲',
  '🧑‍🦼': '轮椅',
  '👨‍🦼': '男轮椅',
  '👩‍🦼': '女轮椅',
  '🧑‍🦽': '轮椅',
  '👨‍🦽': '男轮椅',
  '👩‍🦽': '女轮椅',
  '🏌️': '高尔夫',
  '🏌️‍♀️': '女高尔夫',
  '🏌️‍♂️': '男高尔夫',
  '🏋️': '举重',
  '🏋️‍♀️': '女举重',
  '🏋️‍♂️': '男举重',
  '🏊': '游泳',
  '🏊‍♀️': '女游泳',
  '🏊‍♂️': '男游泳',
  '🏄': '冲浪',
  '🏄‍♀️': '女冲浪',
  '🏄‍♂️': '男冲浪',
  '🚣': '划船',
  '🚣‍♀️': '女划船',
  '🚣‍♂️': '男划船',
  '🧗': '攀岩',
  '🧗‍♀️': '女攀岩',
  '🧗‍♂️': '男攀岩',
  '🚵': '山地车',
  '🚵‍♀️': '女山地车',
  '🚵‍♂️': '男山地车',
  '🚴': '骑车',
  '🚴‍♀️': '女骑车',
  '🚴‍♂️': '男骑车',
  '🏇': '骑马',
  '🏇‍♀️': '女骑马',
  '🏇‍♂️': '男骑马',
  '⛷️': '滑雪',
  '🎿': '滑雪',
  '🏂': '单板滑雪',
  '🤸': '翻跟头',
  '🤸‍♀️': '女翻跟头',
  '🤸‍♂️': '男翻跟头',
  '🤽': '水球',
  '🤽‍♀️': '女水球',
  '🤽‍♂️': '男水球',
  '🤾': '手球',
  '🤾‍♀️': '女手球',
  '🤾‍♂️': '男手球',
  '🤹': '杂耍',
  '🤹‍♀️': '女杂耍',
  '🤹‍♂️': '男杂耍',
  '🎪': '马戏团',
  '🎭': '戏剧',
  '🎨': '艺术',
  '🎬': '电影',
  '🎤': '麦克风',
  '🎧': '耳机',
  '🎼': '乐谱',
  '🎹': '钢琴',
  '🥁': '鼓',
  '🎷': '萨克斯',
  '🎺': '小号',
  '🎸': '吉他',
  '🪕': '班卓琴',
  '🎻': '小提琴',
  '🪗': '手风琴',
  '🎯': '飞镖',
  '🎱': '台球',
  '🎳': '保龄球',
  '🎮': '游戏',
  '🎰': '老虎机',
  '🎲': '骰子',
  '🧩': '拼图',

  '🚗': '汽车',
  '🚕': '出租车',
  '🚙': 'SUV',
  '🚌': '公交车',
  '🚎': '公交车',
  '🏎️': '赛车',
  '🚓': '警车',
  '🚑': '救护车',
  '🚒': '消防车',
  '🚐': '面包车',
  '🚚': '卡车',
  '🚛': '卡车',
  '🚜': '拖拉机',
  '🛵': '电动车',
  '🏍️': '摩托车',
  '🛺': '三轮车',
  '🚲': '自行车',
  '🛴': '滑板车',
  '🚨': '警报',
  '🚔': '警车',
  '🚍': '公交车',
  '🚘': '汽车',
  '🚖': '出租车',
  '🚡': '缆车',
  '🚠': '缆车',
  '🚟': '缆车',
  '🚃': '火车',
  '🚋': '电车',
  '🚞': '火车',
  '🚝': '地铁',
  '🚄': '高铁',
  '🚅': '高铁',
  '🚈': '火车',
  '🚂': '火车',
  '🚆': '火车',
  '🚇': '地铁',
  '🚊': '轻轨',
  '⛽': '加油站',
  '🚧': '施工',
  '🚦': '交通灯',
  '🚥': '交通灯',
  '🛞': '轮胎',
  '🚀': '火箭',
  '🛸': '飞碟',
  '🚁': '直升机',
  '🛩️': '飞机',
  '✈️': '飞机',
  '🛫': '起飞',
  '🛬': '降落',
  '🪂': '降落伞',
  '🚤': '快艇',
  '🛳️': '邮轮',
  '⛴️': '渡轮',
  '🚢': '轮船',
  '⚓': '锚',
  '⛰️': '山',
  '🌋': '火山',
  '🏔️': '雪山',
  '🗻': '富士山',
  '🏕️': '露营',
  '🏖️': '沙滩',
  '🏜️': '沙漠',
  '🏝️': '岛屿',
  '🏞️': '风景',
  '🏟️': '体育场',
  '🏛️': '建筑',
  '🏗️': '施工',
  '🧱': '砖块',
  '🪨': '石头',
  '🪵': '木头',
  '🛖': '小屋',
  '🏘️': '社区',
  '🏚️': '废墟',
  '🏠': '房子',
  '🏡': '房子',
  '🏢': '办公楼',
  '🏬': '商场',
  '🏭': '工厂',
  '🏯': '城堡',
  '🏰': '城堡',
  '💒': '教堂',
  '🗼': '塔',
  '🗽': '自由女神',
  '⛪': '教堂',
  '🕌': '清真寺',
  '🛕': '寺庙',
  '🕍': '犹太教堂',
  '⛩️': '鸟居',
  '🛤️': '轨道',
  '🛢️': '油井',
  '💈': '理发店',
  '⛲': '喷泉',
  '🎡': '摩天轮',
  '🎢': '过山车',
  '🎠': '旋转木马',
  '⛱️': '遮阳伞',
  '🎃': '南瓜灯',
  '🎄': '圣诞树',
  '🎆': '烟花',
  '🎇': '烟花',
  '🧨': '鞭炮',
  '✨': '星星',
  '🎈': '气球',
  '🎉': '庆祝',
  '🎊': '庆祝',
  '🎋': '竹子',
  '🎍': '松树',
  '🎎': '玩偶',
  '🎏': '风筝',
  '🎐': '风铃',
  '🎑': '月亮',
  '🧧': '红包',
  '🎀': '蝴蝶结',
  '🎁': '礼物',
  '🤶': '圣诞奶奶',
  '🎅': '圣诞老人',
  '🦌': '鹿',
  '⛄': '雪人',
  '🔥': '火',
  '❄️': '雪花',
  '☃️': '雪人',
  '🌨️': '下雪',
  '🌧️': '下雨',
  '🌦️': '雨',
  '🌥️': '多云',
  '☁️': '云',
  '🌤️': '晴天',
  '☀️': '太阳',
  '🌝': '月亮',
  '🌞': '太阳',
  '🪐': '行星',
  '🌟': '星星',
  '⭐': '星星',
  '⚡': '闪电',
  '☄️': '彗星',
  '💫': '星星',
  '🌙': '月亮',
  '🌚': '月亮',
  '🌛': '月亮',
  '🌜': '月亮',
  '🌔': '月亮',
  '🌓': '月亮',
  '🌒': '月亮',
  '🌑': '月亮',
  '🌘': '月亮',
  '🌗': '月亮',
  '🌖': '月亮',
  '🌕': '月亮',
  '🌪️': '龙卷风',
  '🌫️': '雾',
  '🌬️': '风',
  '🌀': '漩涡',
  '🌈': '彩虹',
  '🌂': '雨伞',
  '☂️': '雨伞',
  '🌊': '海浪',
  '💧': '水滴',
  '💦': '水滴',
  '☔': '雨伞',

  '🏵️': '花',
  '🎗️': '丝带',
  '🎟️': '票',
  '🎫': '票',
  '🎖️': '奖牌',
  '🏆': '奖杯',
  '🏅': '奖牌',
  '🥇': '金牌',
  '🥈': '银牌',
  '🥉': '铜牌',
  '⚽': '足球',
  '⚾': '棒球',
  '🥎': '垒球',
  '🏀': '篮球',
  '🏐': '排球',
  '🏈': '橄榄球',
  '🏉': '橄榄球',
  '🎾': '网球',
  '🥏': '飞盘',
  '🏏': '板球',
  '🏑': '曲棍球',
  '🏒': '冰球',
  '🥍': '长曲棍球',
  '🏓': '乒乓球',
  '🏸': '羽毛球',
  '🥊': '拳击',
  '🥋': '柔道',
  '🥅': '球门',
  '🎣': '钓鱼',
  '🎽': '运动衫',
  '🛷': '雪橇',
  '🔮': '水晶球',
  '🎵': '音符',
  '🎶': '音乐'
};

const emojiEmotionMap: Record<string, EmojiEmotion> = {
  '😊': 'positive',
  '😃': 'positive',
  '😄': 'positive',
  '😁': 'positive',
  '😆': 'positive',
  '🤣': 'positive',
  '😂': 'positive',
  '🙂': 'positive',
  '🙃': 'positive',
  '😉': 'positive',
  '😌': 'positive',
  '😍': 'positive',
  '🥰': 'positive',
  '😘': 'positive',
  '😚': 'positive',
  '😋': 'positive',
  '😛': 'positive',
  '😝': 'positive',
  '😜': 'positive',
  '🤪': 'positive',
  '😎': 'positive',
  '🤩': 'positive',
  '🥳': 'positive',
  '😏': 'positive',
  '🤓': 'positive',
  '🧐': 'positive',
  '🤨': 'neutral',
  '🤔': 'neutral',
  '🤗': 'positive',
  '🤭': 'positive',
  '😮': 'neutral',
  '😯': 'neutral',
  '😲': 'neutral',
  '😧': 'negative',
  '😦': 'negative',
  '😨': 'negative',
  '😱': 'negative',
  '😖': 'negative',
  '😣': 'negative',
  '😫': 'negative',
  '😩': 'negative',
  '🥺': 'negative',
  '😢': 'negative',
  '😭': 'negative',
  '😤': 'negative',
  '😠': 'negative',
  '😡': 'negative',
  '🤬': 'negative',
  '😳': 'neutral',
  '🥵': 'negative',
  '🥶': 'negative',
  '😴': 'neutral',
  '😪': 'neutral',
  '🤤': 'positive',
  '😓': 'neutral',
  '😟': 'negative',
  '😔': 'negative',
  '😞': 'negative',
  '😒': 'negative',
  '🙁': 'negative',
  '☹️': 'negative',
  '😕': 'neutral',
  '🤫': 'neutral',
  '😶': 'neutral',
  '😐': 'neutral',
  '😑': 'neutral',
  '😬': 'neutral',
  '🙄': 'neutral',
  '😵': 'negative',
  '🤐': 'neutral',
  '🥴': 'negative',
  '🤯': 'negative',
  '🤥': 'negative',
  '🤢': 'negative',
  '🤮': 'negative',
  '🤧': 'negative',
  '😷': 'neutral',
  '🤒': 'negative',
  '🤕': 'negative',
  '🤑': 'positive'
};

module.exports = {
  emojiCategories,
  emojiData,
  emojiTextMap,
  emojiEmotionMap,
  getEmojiText(emoji: string): string {
    return emojiTextMap[emoji] || '';
  },
  getEmojiEmotion(emoji: string): EmojiEmotion {
    return emojiEmotionMap[emoji] || 'neutral';
  },
  getCategoryEmojis(categoryId: string): string[] {
    return emojiData[categoryId] || [];
  },
  getCategories(): EmojiCategory[] {
    return emojiCategories;
  }
};

export {};
